                                ...
                                <?php if ($result->num_rows > 0): ?>
                                    <?php while ($row = $result->fetch_assoc()): ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><strong><?= htmlspecialchars($row['tin']) ?></strong></td>
                                            <td><?= htmlspecialchars($row['payee']) ?></td>
                                            <td>₱<?= number_format($grossAmount, 2) ?></td>
                                            <td>₱<?= number_format($baseAmount, 2) ?></td>
                                            <td>₱<?= number_format($onePercent, 2) ?></td>
                                            <td>₱<?= number_format($fivePercent, 2) ?></td>
                                            <td><strong>₱<?= number_format($totalTax, 2) ?></strong></td>
                                            <td>
                                                <div class="action-buttons">
                                                    <button class="btn btn-sm btn-secondary" onclick="viewBIR(<?= $row['id'] ?>)">
                                                        <i class="fas fa-eye"></i>
                                                    </button>
                                                    <button class="btn btn-sm btn-danger" onclick="deleteBIR(<?= $row['id'] ?>)">
